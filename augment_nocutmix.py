...
source_dir = Path('New_ReLabel')
target_dir = Path('augv5')
...
seq = iaa.SomeOf((1,4), [
    ...
    iaa.Cutout(nb_iterations=(1, 10), size=0.1, squared=False),
...
while True:
    for img,lab in zip(ls_img,ls_lab):
        ...
        new_img_path = Path(target_dir,'images',ty, f'aug_{idx}_'+img.stem + '.jpg')
        cv2.imwrite(str(new_img_path),image_aug,[int(cv2.IMWRITE_JPEG_QUALITY), 100])
        
        new_lab_path = Path(target_dir,'labels',ty, f'aug_{idx}_'+img.stem + '.txt')
